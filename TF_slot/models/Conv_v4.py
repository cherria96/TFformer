...
class DepthConv1d2(L.LightningModule):
    def __init__(self, config, num_levels, kernel_size, dilation_c, scaler):
        super(DepthConv1d2, self).__init__()
        
        self.configs = config

        feature_len = config.enc_in-1 + config.time_len
        ...
        self.dec = DepthwiseNet(small_batch_size, small_batch_size, 
                                num_levels, kernel_size=kernel_size, dilation_c=dilation_c,dec=True)
        ...
        self.get_slot = SlotAttention(
            ...
            iters = 3,
            hidden_dim=config.d_model,   # hidden dimension size of slot attention originally 128s
        )
        
        self.y_attention = AttentionLayer(
                        FullAttention(True, config.factor, attention_dropout = config.dropout, output_attention = False),
                        config.seq_len*feature_len, config.n_heads)
        self.dec_attention = AttentionLayer(
                        FullAttention(False, config.factor, attention_dropout = config.dropout, output_attention = False),
                        config.seq_len*feature_len, config.n_heads)
        self.norm = nn.LayerNorm(config.seq_len*feature_len)
    ...
    def forward(self, x, y, x_mark, y_mark):
        ...
        slot = slot.reshape((-1,1,w*f)).repeat(1,b,1)   # [B*num_slot, b, wf']

        y = y[:,:, :self.configs.label_len,:]
        y = torch.cat((y, torch.zeros((B,b,self.configs.pred_len,f-self.configs.time_len)).type_as(y)),dim=2)
        y_val = torch.cat((y, y_mark), dim=-1)   
        # Reshaping before attention 
        y_val = y_val.reshape((B,b,-1))
        y_out, y_atten = self.y_attention(y_val, y_val,y_val, attn_mask = None)
        y_out = self.norm(y_out)
        # repeat by slot
        y_out = y_out.unsqueeze(1).repeat(1,self.num_slot, 1,1).reshape(-1,b,y_out.shape[-1])
        slot_out, slot_y_val = self.dec_attention(y_out, slot, slot, attn_mask=None) 
        # [B, b, wf']

        # decoder

        dec_out = self.dec(slot_out)   #  [B*num_slot, b, wf'+1]
        dec_out = dec_out.reshape((B, self.num_slot,b, -1))
        out, masks = dec_out.split([w*f,1],dim=-1)   
        # out: [B, num_slot, b, wf]
        # mask: [B, num_slot, b, 1]
        masks = nn.Softmax(dim=1)(masks)
        out = out.reshape((B, self.num_slot, b, w,f))[:,:,:,-self.configs.pred_len:,:self.configs.enc_in-1]
        out = out.reshape((B, self.num_slot, b, -1))  # [B, num_slot, b, wf]
        out_val = out*masks   # [B, num_slot, b, wf]

        out_combine = torch.sum(out_val,dim=1)   # [B, b, wf']
        out_combine = out_combine.reshape((B,b,self.configs.pred_len,-1))   # [B, b, w, f']

        # [B, b, predlen, f]
        # return out_combine[:,:,-self.configs.pred_len:,:self.configs.enc_in-1]
        return out_combine
