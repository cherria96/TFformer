...
class DepthwiseNet(nn.Module):
    def __init__(self, num_inputs,num_outputs, num_levels, kernel_size=2, dilation_c=2):
...
class DepthConv1d(L.LightningModule):
    def __init__(self, config, num_levels, kernel_size, dilation_c, scaler):
        ...
        # self.target=target
        self.dwn = DepthwiseNet(input_len, input_len, num_levels, kernel_size=kernel_size, dilation_c=dilation_c)
        self.dec = DepthwiseNet(input_len, pred_len, num_levels, kernel_size=kernel_size, dilation_c=dilation_c)
    ...
    def forward(self, x):
        ...
        B, b, w, f = x.shape
        # print("Shape:", B,b,w,f)
        y1=self.dwn(x.reshape((-1,w,f)))
        y1 = y1.reshape(x.shape)
        y1 = y1.reshape((B,b, -1))
        # slot attention
        slot = self.get_slot(y1)    # [B, num_slot, wf]
        slot = slot.reshape((-1,w, f)).repeat(1,1,b+1)   # [B*num_slot, w,f]# [B*num_slot, w, f(b+1)]

        # decoder
        dec_out = self.dec(slot)   # [B*num_slot, w, f(b+1)]
        dec_out = dec_out.reshape((B, self.num_slot, self.pred_len,f,b+1))
        out, masks = dec_out.split([b,1],dim=-1)
        # out: [B, num_slot, w,f, b]
        # mask: [B, num_slot, w,f, 1]
        masks = nn.Softmax(dim=1)(masks)
        out_combine = torch.sum(out*masks,dim=1)   # [B, w,f,b]
        out_combine = out_combine.permute(0,3,1,2)  # [B, b, w, f]
