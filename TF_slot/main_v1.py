...
class TimeSeriesTransformer(LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.slot_attention:
            ...
            self.log('Q:', random.randint(100,200))
        else:
            batch['prev_timefeatures'] = batch['prev_timefeatures'].unsqueeze(-1) if batch['prev_timefeatures'].ndim <3 else batch['prev_timefeatures']
            batch['curr_timefeatures'] = batch['curr_timefeatures'].unsqueeze(-1) if batch['curr_timefeatures'].ndim <3 else batch['curr_timefeatures']
            ...
            generated = generated.sequences.cpu().numpy().mean() # or mean?
            loss = self.loss_fn(torch.tensor(generated[0]).to(future_values.device),future_values[0])
            self.log('validation loss', loss, on_step=True, on_epoch=True, prog_bar=True, logger=True)
...
if __name__ == "__main__":
    ...
    slot_attention = True  # True: use our model, False: simpel Timeseries Transformer from hugginface
    if slot_attention:
        model_name = "ours"
    else:
    ...
    wandb_config = {
        'context_length': 60,
        'max_lag': 0,   # size of input value (context length)
        'pred_length': 30,
        'stride': 1,
        'batch_size': 32,
        'epoch': 20,
        ...
        'model': 'tsformer',
        'scaling': None,   # True, "mean", "std" is only meaningful values
    }
    ##################################
